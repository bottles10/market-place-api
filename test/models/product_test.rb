...
class ProductTest < ActiveSupport::TestCase
  test 'should hahve a positive price' do
    product = products(:one)
    product.price = -1
    assert_not product.valid?
  end

  test 'should flter products by name' do
  ...
  test 'should sort product by most recent' do 
    ...
    products(:two).touch(:updated_at)
    # products
    assert_equal [products(:two), products(:one), products(:another_tv)],
      Product.recent.to_a
  end
end
